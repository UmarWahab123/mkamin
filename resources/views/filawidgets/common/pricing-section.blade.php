<!-- PRICING SECTION - DYNAMIC WIDGET
============================================= -->
{{-- @php
...
<div class="{{ isset($paddingTop) ? $paddingTop : 'pt-8' }} pricing-5 pricing-section division">
    <div class="container">
        @if(isset($section->content['showSectionTitle']) ? $section->content['showSectionTitle'] : true)
        <!-- SECTION TITLE -->
        <div class="row justify-content-center">
            <div class="col-lg-10 col-xl-8">
                <div class="section-title text-center mb-6">
                    ...
                    <!-- Title -->
                    <h2 class="h2-title">{{ isset($section->content['sectionTitle']) ? __($section->content['sectionTitle']) : __('Our Services & Prices') }}</h2>
                </div>
            </div>
        </div>
        @endif
                        ...
                        <div class="col-lg-6">
                            <div class="pricing-5-table {{ $loop->even ? 'right-column' : 'left-column' }} wow fadeInUp">
                                ...
                                <ul class="pricing-list">
                                    @if(isset($category->productAndServices) && count($category->productAndServices) > 0)
                ...
                @else
                    ...
                    <div class="col-12 text-center">
                        <div class="alert alert-info">
                            <h4>{{ __('Services Coming Soon') }}</h4>
                            <p>{{ __('We are preparing our services menu. Please check back later.') }}</p>
                        </div>
                    </div>
                @endif
            </div>
        ...
        <div class="row">
            <div class="col">
                <div class="more-btn mt-5">
                    <a href="{{ isset($section->content['buttonUrl']) ? $section->content['buttonUrl'] : (isset($buttonUrl) ? $buttonUrl : route('menu')) }}"
                       class="btn btn--tra-black hover--{{ isset($buttonHoverClass) ? $buttonHoverClass : 'black' }}">
                       {{ isset($section->content['buttonText']) ? __($section->content['buttonText']) : (isset($buttonText) ? __($buttonText) : __('View All Prices')) }}
                    </a>
                </div>
            </div>
        </div>
