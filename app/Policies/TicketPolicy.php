...
class TicketPolicy
{
    ...
    public function view(User $user, Ticket $ticket): bool
    {
        ...
        // POS users can only view their own tickets
        if ($user->hasRole('point_of_sale')) {
            $pointOfSale = PointOfSale::where('user_id', $user->id)->first();
            return $pointOfSale && $ticket->point_of_sale_id === $pointOfSale->id;
        }

        // Staff users can only view their point of sale's tickets
        if ($user->hasRole('staff')) {
            return $user->staff && $ticket->point_of_sale_id === $user->staff->point_of_sale_id;
        }
    ...
    public function update(User $user, Ticket $ticket): bool
    {
        ...
        // POS users can only update their own tickets
        if ($user->hasRole('point_of_sale')) {
            $pointOfSale = PointOfSale::where('user_id', $user->id)->first();
            return $pointOfSale && $ticket->point_of_sale_id === $pointOfSale->id;
        }

        // Staff users can only update their point of sale's tickets
        if ($user->hasRole('staff')) {
            return $user->staff && $ticket->point_of_sale_id === $user->staff->point_of_sale_id;
        }
    ...
    public function delete(User $user, Ticket $ticket): bool
    {
        ...
        // POS users can only delete their own tickets
        if ($user->hasRole('point_of_sale')) {
            $pointOfSale = PointOfSale::where('user_id', $user->id)->first();
            return $pointOfSale && $ticket->point_of_sale_id === $pointOfSale->id;
        }

        // Staff users can only delete their point of sale's tickets
        if ($user->hasRole('staff')) {
            return $user->staff && $ticket->point_of_sale_id === $user->staff->point_of_sale_id;
        }
    ...
    {
        if (!$user->can('force_delete_ticket')) {
            return false;
        }
        ...
        // POS users can only force delete their own tickets
        if ($user->hasRole('point_of_sale')) {
            $pointOfSale = PointOfSale::where('user_id', $user->id)->first();
            return $pointOfSale && $ticket->point_of_sale_id === $pointOfSale->id;
        }

        // Staff users can only force delete their point of sale's tickets
        if ($user->hasRole('staff')) {
            return $user->staff && $ticket->point_of_sale_id === $user->staff->point_of_sale_id;
        }
    ...
    public function restore(User $user, Ticket $ticket): bool
    {
        ...
        // POS users can only restore their own tickets
        if ($user->hasRole('point_of_sale')) {
            $pointOfSale = PointOfSale::where('user_id', $user->id)->first();
            return $pointOfSale && $ticket->point_of_sale_id === $pointOfSale->id;
        }

        // Staff users can only restore their point of sale's tickets
        if ($user->hasRole('staff')) {
            return $user->staff && $ticket->point_of_sale_id === $user->staff->point_of_sale_id;
        }
    ...
    public function replicate(User $user, Ticket $ticket): bool
    {
        ...
        // POS users can only replicate their own tickets
        if ($user->hasRole('point_of_sale')) {
            $pointOfSale = PointOfSale::where('user_id', $user->id)->first();
            return $pointOfSale && $ticket->point_of_sale_id === $pointOfSale->id;
        }

        // Staff users can only replicate their point of sale's tickets
        if ($user->hasRole('staff')) {
            return $user->staff && $ticket->point_of_sale_id === $user->staff->point_of_sale_id;
        }
